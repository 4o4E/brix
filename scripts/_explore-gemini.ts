// brix 开发探索脚本：观察 gemini 对话各阶段的 DOM 状态
//
// 不固化给最终用户用。目的：跑一次画图 + 一次纯文本，每 500ms 抓 DOM 状态，
// 状态变化时打印 + 抓 HTML/截图。用来定位稳定的完成信号、图片元素、下载按钮 selector。
//
// 用法：
//   npx tsx scripts/_explore-gemini.ts
//
// 输出：
...
const log = createLogger('explore-gemini');
...
async function snapshotState(page: Page): Promise<State> {
  return (await page.evaluate(() => {
    ...
    const sendBtn = $('button[aria-label="发送"], button[aria-label="停止响应"]') as HTMLButtonElement | null;
    const sendLabel = sendBtn ? sendBtn.getAttribute('aria-label') : null;
    const sendDisabled = sendBtn ? !!sendBtn.disabled : false;

    const allButtons = $$('button');
    const downloadBtn = allButtons.find((b) => /下载|Download/i.test(b.getAttribute('aria-label') || ''));

    const convCount = $$('.conversation-container, user-query, model-response').length;
    const modelResponseCount = $$('model-response').length;

    const candidateImgs = $$('model-response img, single-image img, image-component img, .response-container img').filter((img) => {
      const el = img as HTMLImageElement;
      const url = el.currentSrc || el.src;
      if (!url) return false;
      if (url.startsWith('data:')) return false;
      if (/avatar|sparkle|gemini_aurora|gstatic\.com\/lamda/i.test(url)) return false;
    ...
    const loadingNodes = $$('.loading-indicator, mat-progress-bar, mat-progress-spinner, .loading-image-container');
    const bodyText = document.body.textContent || '';
    const loadingHintMatch = bodyText.match(/正在创建您的图片|正在创建|正在生成|Generating image|Creating your image/);
    const isLoading = loadingNodes.length > 0 || !!loadingHintMatch;

    const signals: string[] = [];
    const probeTags = ['single-image', 'image-component', 'image-generation', 'message-content', 'response-element',
      'message-actions', 'image-actions', 'attachment-container', 'sources-list', 'response-container'];
    for (const t of probeTags) {
      const n = document.getElementsByTagName(t).length;
      if (n > 0) signals.push(`${t}=${n}`);
    }
    const ariaProbes = ['更多操作', '复制', '点赞', '点踩', '分享', '查看其他草稿', '再生成', '修改', '下载', '保存'];
    ...
    return {
      sendBtnLabel: sendLabel,
      sendBtnDisabled: sendDisabled,
...
function diffStates(a: State, b: State): string[] {
  ...
  const scalarFields: (keyof State)[] = ['sendBtnLabel', 'sendBtnDisabled', 'hasDownloadBtn',
    'conversationCount', 'modelResponseCount', 'imgCount', 'isLoading', 'loadingHint'];
  for (const f of scalarFields) {
    if (JSON.stringify(a[f]) !== JSON.stringify(b[f])) {
      changes.push(`${f}: ${JSON.stringify(a[f])} → ${JSON.stringify(b[f])}`);
    }
  }
  // text length: 只在跨阈值时报告，避免每个 token 都触发
...
async function runOne(page: Page, prompt: string, runDir: string, label: string) {
  ...
  while (Date.now() - t0 < maxMs) {
    ...
    if (cur.sendBtnLabel === '发送' && Date.now() - lastChangeMs > idleMaxMs) {
      log.info(`${label}: response done (send btn restored + ${idleMaxMs}ms idle)`);
      break;
    }
  }
...
async function main() {
  ...
  const page = await newTab('https://gemini.google.com/app');
