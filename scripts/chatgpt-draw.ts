// brix CLI 客户端：通过 HTTP 调用 server 上的 chatgpt-draw 脚本
//
// 用法：
//   BRIX_TOKEN=<token> npm run chatgpt-draw -- "<prompt>" [--out=./out]
//   BRIX_TOKEN=<token> npm run chatgpt-draw -- --json-args '{"prompt":"画一只猫"}' [--out=./out]
//
...
function parseArgv(): { args: unknown; out?: string } {
  ...
  if (!positional[0]) {
    console.error('用法: npm run chatgpt-draw -- "<prompt>" [--out=./out]');
    process.exit(1);
  }
...
async function main() {
  ...
  const result = await runViaBrix('chatgpt-draw', args, { url: 'https://chatgpt.com/', out });
