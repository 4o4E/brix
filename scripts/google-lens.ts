...
//
// 用法（CLI）：
//   npm run lens -- <图片文件 | base64 字符串 | data:image/...;base64,...>
//   npm run lens -- --json-args '{"image":"data:image/png;base64,..."}'
//
// 用法（HTTP via session）：
//   POST /sessions/:sid/scripts/google-lens
...
async function uploadAndExtract(page: Page, imagePath: string, run: Run) {
  ...
  try {
    ...
    if (page.url().includes('/sorry/')) {
      ...
      log.warn('遇到 Google reCAPTCHA，请在浏览器手动完成验证（等待 5 分钟）');
...
export async function runInSession(page: Page, args: unknown, run: Run): Promise<LensOutput> {
  ...
  log.info(`runId=${run.runId} bytes=${bytes.length}`);

  // 把图存到 run.dir 当临时上传源（不进 downloads/）
  ...
  log.info(`done in ${output.durationMs}ms, ${pages.length} item(s)`);
  return output;
}
...
if (import.meta.url === pathToFileURL(process.argv[1]).href) {
  await runAsCli(runInSession, {
    parseArgv: (positional) => {
      if (!positional[0]) {
        console.error('用法: npm run lens -- <图片文件 | base64 | data:image/...;base64,...>');
        process.exit(1);
      }
      return { image: positional[0] };
    },
  });
}
