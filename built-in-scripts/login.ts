...
//
// 执行入口：POST /sessions/:sid/scripts/login
//
// 行为：打开 accounts.google.com，轮询登录状态。
...
export async function runInSession(page: Page, _args: unknown, run: Run): Promise<LoginOutput> {
  const t0 = Date.now();
  const maxMs = 600_000;
  run.log.info('opening accounts.google.com');

  await page.goto('https://accounts.google.com/', { waitUntil: 'domcontentloaded', timeout: 30_000 });
  run.log.info('已打开 accounts.google.com — 请在浏览器内完成登录（最多等 10 分钟）');

  const start = Date.now();
  while (Date.now() - start < maxMs) {
    if (await isLoggedIn(page)) {
      const out: LoginOutput = { loggedIn: true, finalUrl: page.url(), durationMs: Date.now() - t0 };
      run.log.info(`logged in (${out.finalUrl}) in ${out.durationMs}ms`);
      return out;
    }
    await page.waitForTimeout(2_000);
  }

  run.log.warn(`login timeout after ${maxMs}ms, returning loggedIn=false`);
